...
public class FileUpLoad extends HttpServlet {
	...
	{  
		try
		{
			...
			//byte[] data = request.getParameter("imgInfo").getBytes("ISO-8859-1");  
			...
			finalfilename=sdf.format(new Date())+"."+filename.split("\\.")[1];
			// 将路径和上传文件名组合成完整的服务端路径
			filename = uploadPath + finalfilename;
		...
		/*upload.setImginfo(title);
		upload.setImgurl(filename);
		upload.setLatitude(lat);
		upload.setLongitude(lon);
